  ...
  return (
    <nav className="bg-[#0F172A] shadow-lg shadow-green-500/50 fixed top-0 left-0 w-full z-50">
      <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
        {/* Logo / Title */}
        <Link to="/" className="text-2xl sm:text-3xl font-extrabold text-green-400 tracking-tight">
          🛒 ShoppyGlobe
        </Link>

        {/* Desktop Links */}
        <div className="hidden md:flex space-x-7 text-base font-semibold">
          <Link to="/" className="text-gray-300 hover:text-[#88BDF2] transition">
            Home
          </Link>
          <Link
            to="/cart"
      ...
      {isOpen && (
        <div className="md:hidden px-4 pb-4 space-y-2 text-base font-semibold">
          <Link to="/" className="block text-gray-300 hover:text-[#88BDF2]" onClick={() => setIsOpen(false)}>
            Home
          </Link>
          <Link to="/cart" className="block text-gray-300 hover:text-[#88BDF2]" onClick={() => setIsOpen(false)}>
            Cart
          </Link>
          <Link to="/login" className="block text-gray-300 hover:text-[#88BDF2]" onClick={() => setIsOpen(false)}>
            Login
          </Link>
        </div>
      )}
